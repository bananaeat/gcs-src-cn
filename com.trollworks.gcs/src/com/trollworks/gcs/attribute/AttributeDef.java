...
public class AttributeDef implements Cloneable, Comparable<AttributeDef> {
    ...
    public static final Map<String, AttributeDef> createStandardAttributes() {
        ...
        thresholds.add(new PoolThreshold(-1, 1, 0, I18n.text("失去意识"), "", ops));
        thresholds.add(new PoolThreshold(0, 1, 0, I18n.text("累到崩溃"), I18n.text("""
                做Will检定来进行任何谈话和休息以外的行为；检定失败会导致失去意识
                你在0以下失去的每一点FP都会导致失去1点HP
                移动力，闪避和ST减半 (B426)"""), ops));
        thresholds.add(new PoolThreshold(1, 3, 0, I18n.text("疲惫"),
                I18n.text("移动力，闪避和ST减半 (B426)"), ops));
        thresholds.add(new PoolThreshold(1, 1, -1, I18n.text("有些累"), "", null));
        thresholds.add(new PoolThreshold(1, 1, 0, I18n.text("休息充分"), "", null));

        m.put("fp", new AttributeDef("fp", I18n.text("FP"), I18n.text("体力值"), "$ht", ++i,
                3, 0, thresholds));
        ...
        thresholds.add(new PoolThreshold(-5, 1, 0, I18n.text("死亡"), "", ops));
        for (int j = -4; j < 0; j++) {
            thresholds.add(new PoolThreshold(j, 1, 0, String.format(I18n.text("濒死 #%d"),
                    Integer.valueOf(-j)), String.format(I18n.text("""
                    做HT检定来避免死亡
                    每秒做HT %d 检定来避免失去意识
                    移动力和闪避减半 (B419)"""), Integer.valueOf(j)), ops));
        }
        thresholds.add(new PoolThreshold(0, 1, 0, I18n.text("受伤崩溃"), I18n.text("""
                    每秒做HT %d 检定来避免失去意识
                    移动力和闪避减半 (B419)"""), ops));
        thresholds.add(new PoolThreshold(1, 3, 0, I18n.text("蹒跚"),
                I18n.text("移动力和闪避减半 (B419)"), ops));
        thresholds.add(new PoolThreshold(1, 1, -1, I18n.text("受伤"), "", null));
        thresholds.add(new PoolThreshold(1, 1, 0, I18n.text("健康"), "", null));
        m.put("hp", new AttributeDef("hp", I18n.text("HP"), I18n.text("生命值"), "$st", ++i, 2,
                10, thresholds));
